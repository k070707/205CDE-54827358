<?php
	...
	$sql = "SELECT product_name FROM product WHERE product_no=10002";
    $result= $mysqli->query($sql);
	$row = mysqli_fetch_array($result);
	$pname=$row[0];

	$sql = "SELECT price FROM product WHERE product_no=10002";
    $result= $mysqli->query($sql);
	$row = mysqli_fetch_array($result);
	$price=$row[0];
	
	$sql = "INSERT INTO purchas_car (product_no, product_name, price, id) VALUES (10002, ?, ?, ?)";
